...
STAILQ_HEAD (eNB_list_s, eNB_description_s)
  ...
  void *args);
     ...
     static int                              s1ap_send_init_sctp (
  void)
{
  ...
  void *args)
{
  ...
  while (1) {
    ...
    case NAS_DOWNLINK_DATA_REQ:{
         ...
         */
        s1ap_generate_downlink_nas_transport (NAS_DL_DATA_REQ (received_message_p).UEid, NAS_DL_DATA_REQ (received_message_p).nasMsg.data, NAS_DL_DATA_REQ (received_message_p).nasMsg.length);
      }
      break;
  ...
  const uint32_t mme_ue_s1ap_id)
{
  ue_description_t                       *ue_ref;
  eNB_description_t                      *eNB_ref;
  printf("We are inside s1ap_is_ue_mme_id_in_list\n");
  STAILQ_FOREACH (eNB_ref, &eNB_list_head, eNB_entries) {
    STAILQ_FOREACH (ue_ref, &eNB_ref->ue_list_head, ue_entries) {
      // We found a matching reference, return it
      printf("ue_ref->mme_ue_s1ap_id = %x and mme_ue_s1ap_id = %x \n",ue_ref->mme_ue_s1ap_id,mme_ue_s1ap_id);
